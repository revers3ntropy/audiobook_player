...
class Buttons:
    ...
    def display(self, message):
        writing.write(self.font, message, (self.x, self.y))
...
class StandardButton(Buttons):
    ...
    def __init__(self, x, y, font, message):
        ...
        self.size_x = (writing.fonts[font][writing.size_x] + 5) * len(message)
        self.hit_box = (
            x - self.size_x / 2 - 7, y - writing.fonts[font][writing.size_y] / 2 - 7,
            self.size_x + 14,
            writing.fonts[font][writing.size_y] + 14)
...
class SwitchButton(StandardButton):
    ...
    def __init__(self, x, y, font, states, starting_state):
        super().__init__(x, y, font, states[starting_state])
    ...
    def update_state(self):
        if self.number_of_states > 2:
            if self.current_state >= self.number_of_states - 1:
                ...
                self.current_state += 1
            self.size_x = (writing.fonts[self.font][writing.size_x] + 5) * len(
                self.states[self.current_state])
            self.hit_box = (
                self.x - self.size_x / 2, self.y - writing.fonts[self.font][writing.size_y] / 2,
                self.size_x,
                writing.fonts[self.font][writing.size_y])
        else:
...
class TextButton(Buttons):
    ...
    def __init__(self, x, y, font, initial_message, colour1, colour2, max_length, selected_colour):
        super().__init__(x, y, font)
        self.size_x = (writing.fonts[font][writing.size_x] + 5) * max_length
        self.hit_box = (
            self.x - self.size_x / 2 - 5, self.y - writing.fonts[self.font][writing.size_y] / 2 - 5,
            self.size_x + 10,
            writing.fonts[self.font][writing.size_y] + 10)
    ...
    def check_message(self):
        if self.selected and global_data.typing_sticky_keys <= 0:
            ...
            for i in range(len(keys)):
                try:
                    if keys[writing.typeable_characters_py_game[i]]:
                        character = writing.typeable_characters[i]
    ...
    def check_message(self):
        if self.selected:
            ...
            for i in range(len(keys)):
                try:
                    if keys[writing.typeable_characters_py_game[i]]:
                        if global_data.typing_last_key != writing.typeable_characters_py_game[i] or \
                                global_data.typing_sticky_keys <= 0:
                            character = writing.typeable_characters[i]
                            global_data.typing_sticky_keys = 20
                            global_data.typing_last_key = writing.typeable_characters_py_game[i]
                except IndexError:
                    pass
